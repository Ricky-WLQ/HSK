...
export default async function TranslationSetList({
  params,
}: {
  ...
  const sets = await getPracticeSetsForLevel(level, "translation");
    ...
    <div className="min-h-screen">
      ...
      <main className="container-app py-10">
        ...
        <h1 className="font-heading mt-1 text-3xl font-extrabold">
          {t.practice.level} {level} · {t.practice.translation}
        </h1>

        {sets.length === 0 ? (
          <p className="mt-6 text-foreground/75">{t.practice.noTranslationSets}</p>
        ) : (
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            {sets.map((s) => (
              <Link
                key={s.id}
                href={`/practice/${level}/translation/${s.id}`}
