...
export default async function WritingSetList({
  params,
}: {
  ...
  const sets = await getPracticeSetsForLevel(level, "writing");
    ...
    <div className="min-h-screen">
      ...
      <main className="container-app py-10">
        ...
        <h1 className="font-heading mt-1 text-3xl font-extrabold">
          {t.practice.level} {level} · {t.practice.writing}
        </h1>
        ...
        {sets.length === 0 ? (
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            {sets.map((s) => (
              <Link
                key={s.id}
                href={`/practice/${level}/writing/${s.id}`}
